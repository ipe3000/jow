...
const TOP3_SWEEP_VP=DIAMOND_VP_AWARDS.reduce((sum,v)=>sum+v,0);
const MILITARY_VP=2;
...
function checkSupremacy(S){const f0=S.players[0].feat, f1=S.players[1].feat; if(f0.sw-f1.sw>=8) return {winner:0,reason:"Military Supremacy"}; if(f1.sw-f0.sw>=8) return {winner:1,reason:"Military Supremacy"}; if(f0.cw-f1.cw>=8) return {winner:0,reason:"Food Supremacy"}; if(f1.cw-f0.cw>=8) return {winner:1,reason:"Food Supremacy"}; return null;}
...
function choosePlayoutMove(S,eps=0.2,mode="insta"){
 ...
 for(const idx of moves){
  const v=mode==="total_black"?totalBlackTakeValue(S,S.current,idx):cheapEvalTake(S,S.current,idx);
...
function randomPlayout(S,perspective=S.current,mcCfg=DEFAULT_MC_CFG,rewardMode="insta"){let guard=600; while(!S.ended && guard-->0){const moves=legalMoves(S); if(!moves.length){S.picksLeftThisTurn=1; S.current=1-S.current; continue;} if(shouldUseJokerInPlayout(S)){S.players[S.current].joker=false;S.picksLeftThisTurn=2;} const idx=choosePlayoutMove(S,mcCfg.eps,rewardMode); if(idx===null){S.picksLeftThisTurn=1;S.current=1-S.current;continue;} applyTake(S,idx,mcCfg);} return finalReward(S,perspective,rewardMode);}
function chooseMoveUcb(S,moves,mcCfg,{topK=3,rewardMode="insta",exploration=0.9,tBias=1}={}){
 const ordered=moves
  .map(i=>({idx:i,v:rewardMode==="total_black"?totalBlackTakeValue(S,S.current,i):cheapEvalTake(S,S.current,i)}))
  .sort((a,b)=>b.v-a.v);
...
async function runBatch(){const n=Math.max(1,Math.min(10000,Number(document.getElementById("games").value)||1000)); const sA=document.getElementById("playerStrategy").value, sB=document.getElementById("aiStrategy").value, sm=document.getElementById("startMode").value, mcCfg=mcSettings(readMcStrength());
 ...
 const pointsOnlyTop3SweepBonusGames=pointsOnly.filter(x=>
  x.score.tech.some(v=>v===TOP3_SWEEP_VP) || x.score.culture.some(v=>v===TOP3_SWEEP_VP)
 ).length;
